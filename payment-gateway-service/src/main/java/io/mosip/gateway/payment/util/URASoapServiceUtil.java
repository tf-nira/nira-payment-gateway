...
import io.mosip.gateway.payment.dto.ura.URASoapGetAssessmentResponseDTO;
...
public class URASoapServiceUtil {
	...
	@Value("${ura.action.get-assessment-amount}")
	private String uraGetAssessmentAmountAction;
	...
	@Value("${ura.taxheadcode.conversion}")
	private String uraTaxHeadCodeForConversion;
	...
	public String getPRN(URASoapGeneratePRNRequestDTO requestDTO) {
		...
		log.debug("URA Get PRN Request Payload: {}", soapRequest);
	...
	public String getAssessmentAmount(URASoapGeneratePRNRequestDTO requestDTO) {

	    String soapRequest = buildGetAssessmentRequest(requestDTO);
	    
	    log.debug("URA Get Assessment Amount SOAP Request Payload: {}", soapRequest);

	    HttpHeaders headers = new HttpHeaders();
	    headers.setContentType(MediaType.TEXT_XML);
	    headers.add("SOAPAction", uraGetAssessmentAmountAction);
	...
	private String buildGetAssessmentRequest(URASoapGeneratePRNRequestDTO requestDTO) {
	    ...
	    soapRequest.append("<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" ")
	            ...
	            .append("<soapenv:Body>")
	            .append("<tem:GetAssessmentAmount>")
	            .append("<tem:ntrRequest>")
	            .append("<urap:GetAssessmentDataContract.GetNTRAssessment>")

	            .append("<urap:AdditionalFees>0</urap:AdditionalFees>")
	            .append("<urap:GrossAmount>").append(requestDTO.getAmount()).append("</urap:GrossAmount>")
	            .append("<urap:NTRTaxHeadCode>").append(uraTaxHeadCodeForConversion).append("</urap:NTRTaxHeadCode>")
	            .append("<urap:NoForms>1</urap:NoForms>")
...
//	}
	...
	public URANTRAssessmentDTO parseGetAssessmentResponse(String response) throws Exception {
	    ...
	    URASoapGetAssessmentResponseDTO dto =
	            xmlMapper.readValue(soapBodyContent, URASoapGetAssessmentResponseDTO.class);

	    return dto.getResult().getAssessment();
	}
